...
#define PN532DEBUG
#define MIFAREDEBUG
#define PN532DEBUGPRINT ESP_LOGI
...
uint32_t pn532_getFirmwareVersion(pn532_t *obj)
{
    ...
    {
#ifdef PN532DEBUG
        PN532DEBUGPRINT(TAG, "Firmware doesn't match!");
#endif
        return 0;
    }
...
bool pn532_sendCommandCheckAck(pn532_t *obj, uint8_t *cmd, uint8_t cmdlen, uint16_t timeout)
{
...
#ifdef PN532DEBUG
    PN532DEBUGPRINT(TAG, "IRQ received");
#endif
    ...
    {
#ifdef PN532DEBUG
        PN532DEBUGPRINT(TAG, "No ACK frame received!");
#endif
        return false;
    }
...
#ifdef PN532DEBUG
    PN532DEBUGPRINT(TAG, "Writing P3 GPIO: 0x%x", pn532_packetbuffer[1]);
#endif
...
#ifdef PN532DEBUG
    PN532DEBUGPRINT(TAG, "Received");
    esp_log_buffer_hexdump_internal(TAG, pn532_packetbuffer, 8, ESP_LOG_INFO);
#endif
...
#ifdef PN532DEBUG
    PN532DEBUGPRINT(TAG, "Received");
    esp_log_buffer_hexdump_internal(TAG, pn532_packetbuffer, 11, ESP_LOG_INFO);
    
    PN532DEBUGPRINT(TAG, "P3 GPIO: 0x%x", pn532_packetbuffer[p3offset]);
    PN532DEBUGPRINT(TAG, "P7 GPIO: 0x%x", pn532_packetbuffer[p3offset + 1]);
    PN532DEBUGPRINT(TAG, "IO GPIO: 0x%x", pn532_packetbuffer[p3offset + 2]);    
    // Note: You can use the IO GPIO value to detect the serial bus being used
    switch (pn532_packetbuffer[p3offset + 2])
    {
    case 0x00: // Using UART
        PN532DEBUGPRINT(TAG, "Using UART (IO = 0x00)");
        break;
    case 0x01: // Using I2C
        PN532DEBUGPRINT(TAG, "Using I2C (IO = 0x01)");
        break;
    case 0x02: // Using SPI
        PN532DEBUGPRINT(TAG, "Using SPI (IO = 0x02)");
        break;
    }
#endif
...
#ifdef MIFAREDEBUG
    PN532DEBUGPRINT(TAG, "Setting MxRtyPassiveActivation to %d", maxRetries);
#endif
...
bool pn532_readPassiveTargetID(pn532_t *obj, uint8_t cardbaudrate, uint8_t *uid, uint8_t *uidLength, uint16_t timeout)
{
    ...
    {
#ifdef PN532DEBUG
        PN532DEBUGPRINT(TAG, "No card(s) read");
#endif
    ...
    {
#ifdef PN532DEBUG
        PN532DEBUGPRINT(TAG, "Waiting for IRQ (indicates card presence)");
#endif
        if (!pn532_waitready(obj, timeout))
        {
#ifdef PN532DEBUG
            PN532DEBUGPRINT(TAG, "IRQ Timeout");
#endif
            return 0x0;
        }
    }
...
#ifdef MIFAREDEBUG
    PN532DEBUGPRINT(TAG, "Found %d tags", pn532_packetbuffer[7]);
#endif
...
#ifdef MIFAREDEBUG
    PN532DEBUGPRINT(TAG, "ATQA: 0x%x", sens_res);
    PN532DEBUGPRINT(TAG, "SAK: 0x%x", pn532_packetbuffer[11]);
#endif
...
#ifdef MIFAREDEBUG
    PN532DEBUGPRINT(TAG, "UID");
    esp_log_buffer_hexdump_internal(TAG, uid, pn532_packetbuffer[12], ESP_LOG_INFO);
#endif
...
bool pn532_inDataExchange(pn532_t *obj, uint8_t *send, uint8_t sendLength, uint8_t *response, uint8_t *responseLength)
{
    ...
    {
#ifdef PN532DEBUG
        PN532DEBUGPRINT(TAG, "APDU length too long for packet buffer");
#endif
        return false;
    }
    uint8_t i;
    ...
    {
#ifdef PN532DEBUG
        PN532DEBUGPRINT(TAG, "Could not send APDU");
#endif
        return false;
    }
    ...
    {
#ifdef PN532DEBUG
        PN532DEBUGPRINT(TAG, "Response never received for APDU...");
#endif
        return false;
    }
    ...
    if (pn532_packetbuffer[0] == 0 && pn532_packetbuffer[1] == 0 && pn532_packetbuffer[2] == 0xff)
    {
        ...
        {
#ifdef PN532DEBUG
            PN532DEBUGPRINT(TAG, "Length check invalid %x%x", length, (~length) + 1);
#endif
            return false;
        }
        if (pn532_packetbuffer[5] == PN532_PN532TOHOST && pn532_packetbuffer[6] == PN532_RESPONSE_INDATAEXCHANGE)
        {
            ...
            {
#ifdef PN532DEBUG
                PN532DEBUGPRINT(TAG, "Status code indicates an error");
#endif
                return false;
            }
        ...
        {
            PN532DEBUGPRINT(TAG, "Don't know how to handle this command: 0x%x", pn532_packetbuffer[6]);
            return false;
        }
    }
    else
    {
        PN532DEBUGPRINT(TAG, "Preamble missing");
        return false;
    }
}
...
bool pn532_inListPassiveTarget(pn532_t *obj)
{
...
#ifdef PN532DEBUG
    PN532DEBUGPRINT(TAG, "About to inList passive target");
#endif

    if (!pn532_sendCommandCheckAck(obj, pn532_packetbuffer, 3, 1000))
    {
#ifdef PN532DEBUG
        PN532DEBUGPRINT(TAG, "Could not send inlist message");
#endif
        return false;
    }
    ...
    if (pn532_packetbuffer[0] == 0 && pn532_packetbuffer[1] == 0 && pn532_packetbuffer[2] == 0xff)
    {
        ...
        if (pn532_packetbuffer[5] == PN532_PN532TOHOST && pn532_packetbuffer[6] == PN532_RESPONSE_INLISTPASSIVETARGET)
        {
            ...
            {
#ifdef PN532DEBUG
                PN532DEBUGPRINT(TAG, "Unhandled number of targets inlisted");
#endif
                PN532DEBUGPRINT(TAG, "Number of tags inlisted: %d", pn532_packetbuffer[7]);
                return false;
            }

            obj->_inListedTag = pn532_packetbuffer[8];
            PN532DEBUGPRINT(TAG, "Tag number: %d", obj->_inListedTag);
        ...
        {
#ifdef PN532DEBUG
            PN532DEBUGPRINT(TAG, "Unexpected response to inlist passive host");
#endif
            return false;
        }
    }
...
uint8_t pn532_mifareclassic_AuthenticateBlock(pn532_t *obj, uint8_t *uid, uint8_t uidLen, uint32_t blockNumber, uint8_t keyNumber, uint8_t *keyData)
{
...
#ifdef MIFAREDEBUG
    PN532DEBUGPRINT(TAG, "Trying to authenticate card");
    esp_log_buffer_hexdump_internal(TAG, obj->_uid, obj->_uidLen, ESP_LOG_INFO);
    PN532DEBUGPRINT(TAG, "Using authentication KEY %c", keyNumber ? 'B' : 'A');
    ...
    {
#ifdef PN532DEBUG
        PN532DEBUGPRINT(TAG, "Authentification failed");
...
{
#ifdef MIFAREDEBUG
    PN532DEBUGPRINT(TAG, "Trying to read 16 bytes from block %d", blockNumber);
#endif
    ...
    {
#ifdef MIFAREDEBUG
        PN532DEBUGPRINT(TAG, "Failed to receive ACK for read command");
#endif
        return 0;
    }
    ...
    {
#ifdef MIFAREDEBUG
        PN532DEBUGPRINT(TAG, "Unexpected response");
        esp_log_buffer_hexdump_internal(TAG, pn532_packetbuffer, 26, ESP_LOG_INFO);
#endif
        return 0;
    }
...
#ifdef MIFAREDEBUG
    PN532DEBUGPRINT(TAG, "Block %d", blockNumber);
...
{
#ifdef MIFAREDEBUG
    PN532DEBUGPRINT(TAG, "Trying to write 16 bytes to block %d", blockNumber);
#endif
    ...
    {
#ifdef MIFAREDEBUG
        PN532DEBUGPRINT(TAG, "Failed to receive ACK for write command");
#endif
        return 0;
    }
    PN532_DELAY(10);
...
uint8_t pn532_mifareultralight_ReadPage(pn532_t *obj, uint8_t page, uint8_t *buffer)
{
    ...
    {
#ifdef MIFAREDEBUG
        PN532DEBUGPRINT(TAG, "Page value out of range");
#endif
        return 0;
    }

#ifdef MIFAREDEBUG
    PN532DEBUGPRINT(TAG, "Reading page %d", page);
#endif
    ...
    {
#ifdef MIFAREDEBUG
        PN532DEBUGPRINT(TAG, "Unexpected response reading block");
        esp_log_buffer_hexdump_internal(TAG, pn532_packetbuffer, 26, ESP_LOG_INFO);
#endif
        return 0;
    }
...
/* Display data for debug if requested */
#ifdef MIFAREDEBUG
    PN532DEBUGPRINT(TAG, "Page %d", page);
    esp_log_buffer_hexdump_internal(TAG, buffer, 4, ESP_LOG_INFO);
#endif
...
#ifdef MIFAREDEBUG
    PN532DEBUGPRINT(TAG, "Trying to write 4 uint8_t page %d", page);
#endif
...
uint8_t pn532_getDataTarget(pn532_t *obj, uint8_t *cmd, uint8_t *cmdlen)
{
    ...
    if (!pn532_sendCommandCheckAck(obj, pn532_packetbuffer, 1, 1000))
    {
        PN532DEBUGPRINT(TAG, "Error en ack");
        return false;
    }
...
void pn532_writecommand(pn532_t *obj, uint8_t *cmd, uint8_t cmdlen)
{
...
#ifdef PN532DEBUG
    PN532DEBUGPRINT(TAG, "Sending");
#endif

    gpio_set_level(obj->_ss, 0);
    PN532_DELAY(2); // or whatever the PN532_DELAY is for waking up the board
...
#ifdef PN532DEBUG
    PN532DEBUGPRINT(TAG, "0x%x", (uint8_t)PN532_PREAMBLE);    
    PN532DEBUGPRINT(TAG, "0x%x", (uint8_t)PN532_PREAMBLE);
    PN532DEBUGPRINT(TAG, "0x%x", (uint8_t)PN532_STARTCODE2);
    PN532DEBUGPRINT(TAG, "0x%x", (uint8_t)cmdlen);
    PN532DEBUGPRINT(TAG, "0x%x", (uint8_t)(~cmdlen + 1));
    PN532DEBUGPRINT(TAG, "0x%x", (uint8_t)PN532_HOSTTOPN532);
#endif
    ...
    for (uint8_t i = 0; i < cmdlen - 1; i++)
    {
        ...
        checksum += cmd[i];
#ifdef PN532DEBUG
        PN532DEBUGPRINT(TAG, "0x%x", (uint8_t)cmd[i]);
#endif
    }
...
#ifdef PN532DEBUG
    PN532DEBUGPRINT(TAG, "0x%x", (uint8_t)~checksum);
    PN532DEBUGPRINT(TAG, "0x%x", (uint8_t)PN532_POSTAMBLE);
#endif
}
/************** low level SPI */
